...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Starter Pack Blender processor")
    parser.add_argument("--blender", help="Path to blender.exe (or 'blender' if on PATH)", default=None)
    parser.add_argument("--figure", required=False, help="Path to base_character_3d.* file")
    parser.add_argument("--acc", nargs="*", default=[], help="Accessory files (accessory_1_3d.*, etc.)")
    parser.add_argument("--outdir", required=False, default="./blender_processor_out", help="Output root folder")
    parser.add_argument("--jobid", required=True, help="Job identifier (folder name)")
    parser.add_argument("--timeout", type=int, default=300, help="Blender timeout seconds")
    parser.add_argument("--justtest", action="store_true", help="Only run health check and exit")

    args = parser.parse_args()

    # Allow the simple “just test” mode you saw earlier
    if args.justtest and not args.figure and not args.acc:
        asyncio.run(test_blender_installation())
        sys.exit(0)

    # ---- Override settings from CLI so the class uses your values ----
    if args.blender:
        settings.BLENDER_EXECUTABLE = args.blender  # e.g. "blender.exe" or full path
    settings.BLENDER_TIMEOUT = args.timeout

    # The processor writes to: settings.PROCESSED_PATH / <jobid> / final
    # Point that root at your --outdir
    if args.outdir:
        settings.PROCESSED_PATH = os.path.abspath(args.outdir)

    # Sanity prints
    print(f"✅ Blender processor initialized - Executable: {settings.BLENDER_EXECUTABLE}")
    print("🧪 Testing Blender installation...")
    ok = asyncio.run(BlenderProcessor().health_check())
    if not ok:
        print("❌ Blender health check failed. Aborting.")
        sys.exit(1)
    # optional STL smoke test (comment out if you don’t want it)
    # tmp_stl = os.path.join(tempfile.gettempdir(), "blender_test.stl")
    # asyncio.run(BlenderProcessor().create_simple_test_stl(tmp_stl))
    # if os.path.exists(tmp_stl): os.remove(tmp_stl)

    # ---- Build the models list the processor expects ----
    models_3d = []
    if args.figure:
        models_3d.append({"model_path": os.path.abspath(args.figure)})
    for p in args.acc:
        models_3d.append({"model_path": os.path.abspath(p)})

    if not models_3d:
        print("❌ No input models provided. Use --figure and/or --acc.")
        sys.exit(2)

    # ---- Run the full pipeline (regular + keychain) ----
    proc = BlenderProcessor()
    result = asyncio.run(proc.process_3d_models(args.jobid, models_3d))
    ...
    for f in result.get("output_files", []):
        print(f" • {f['filename']}  ({f['file_size_mb']} MB)  → {f['file_path']}")
