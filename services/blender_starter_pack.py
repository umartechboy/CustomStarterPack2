...
def parse_args():
    ...
    p.add_argument("--figure_depth", default=None, help="Path to figure depth PNG (optional)")
    p.add_argument("--figure_img", default=None, help="Path to figure color PNG (optional)")
...
def main():
    ...
    # Step 2: Create and position figure (optional — skipped when PrintMaker handles figure)
    figure_top_z = None
    figure = None
    figure_pos = None
    figure_dims = None
    figure_crop_ratios = None
    if args.figure_depth and args.figure_img and os.path.exists(args.figure_depth) and os.path.exists(args.figure_img):
        figure = create_displaced_mesh(args.figure_depth, args.figure_img, name="Figure",
                                       displacement_strength=args.displacement_figure)
    else:
        print("  Skipping figure — not provided (handled separately by PrintMaker)")
    if figure:
